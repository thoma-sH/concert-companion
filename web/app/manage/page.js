...
export default function ManagePage() {
  ...
  const updateSharedColor = useCallback(
    (color) => {
      ...
      fetch("/api/venue/concert/setcolor", {
        method: "POST",
        body: JSON.stringify({ concertId: concertId, color: color_code }),
      });
    },
    [concertId]
  );
    ...
    (type) => {
      ...
      if (type === "strobe") {
        ...
        effectRef.current = setInterval(() => {
          ...
          updateSharedColor(
            `rgb(${Math.min(255, Math.floor(r0 * finalFactor))}, ${Math.min(255, Math.floor(g0 * finalFactor))}, ${Math.min(255, Math.floor(b0 * finalFactor))})`
          );
          step++;
        }, getDelay(100));
      ...
      <div className="max-w-5xl mx-auto p-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-6">
          <div>
            ...
            {concert && (
              <p className="text-sm text-[#b0d4ff] mt-1">{String(new Date(concert.StartDate))}</p>
            )}
          </div>
          <button
          ...
          {["lightsync", "chat", "reports"].map((tab) => (
            <button
              ...
              className={`px-4 py-2 capitalize transition ${
                activeTab === tab
                  ? "border-b-2 border-[#38b6ff] text-[#38b6ff]"
            ...
            <div className="bg-[#0a1f3d]/60 rounded-xl p-6 border border-[#38b6ff]/20">
              ...
              <div
                ...
                style={{
                  ...
                  transition: activeEffect === "strobe" ? "none" : "background-color 80ms ease",
                }}
              />
              <p className="text-center text-sm mt-2 text-gray-300">
                {activeEffect ? `Effect: ${activeEffect} running` : "Static color"}
              </p>
            </div>
        ...
        {activeTab === "chat" && (
          <div className="bg-[#0a1f3d]/60 rounded-xl p-4 border border-[#38b6ff]/20">
            <div className="h-80 overflow-y-auto space-y-2 mb-4">
              ...
              {messages.map((msg) => (
                <div
                  ...
                  className="bg-black/30 p-2 rounded flex justify-between items-start group"
                >
                  <div className="flex-1">
            ...
            <div className="flex gap-2">
              ...
              <button
                onClick={sendMessage}
                className="px-4 py-2 rounded bg-[#38b6ff] text-black font-semibold"
              >
                Send
              </button>
            </div>
