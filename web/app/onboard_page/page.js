    ...
    <div className="min-h-screen bg-gradient-to-br from-[#050d1a] via-[#071628] to-[#0a1f3d] flex items-center justify-center p-6">
      <div className="max-w-md w-full bg-[#0a1f3d]/60 backdrop-blur-sm rounded-2xl p-6 border border-[#38b6ff]/20 shadow-xl">
        ...
        <div className="flex gap-2 mb-6 bg-[#071628] rounded-full p-1">
          <button
            ...
            className={`flex-1 py-2 rounded-full text-sm font-medium transition ${
              activeTab === "signup" ? "bg-[#38b6ff] text-[#050d1a]" : "text-[#b0d4ff] hover:text-white"
            }`}
          >
            Sign Up
          </button>
          <button
            ...
            className={`flex-1 py-2 rounded-full text-sm font-medium transition ${
              activeTab === "login" ? "bg-[#38b6ff] text-[#050d1a]" : "text-[#b0d4ff] hover:text-white"
            }`}
          >
            Log In
          </button>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* 🧪 DEV LOGIN BUTTON - always visible in development */}
          {process.env.NODE_ENV === "development" && (
            <button
              type="button"
              onClick={handleDevLogin}
              className="w-full mt-2 py-2 rounded-full border border-yellow-500 text-yellow-400 text-sm font-medium hover:bg-yellow-500/10 transition"
            >
              🧪 Dev Login (instant, no email/password)
            </button>
          )}
        </form>
